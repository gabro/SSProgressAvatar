...
@interface SSProgessAvatar ()
@property (nonatomic, strong) NSTimer * indefiniteTimer;
@end

@implementation SSProgessAvatar
...
- (void)commonInit {
    ...
    _outerBorderColor = [UIColor blueColor];
...
- (void)setIndefinite:(BOOL)indefinite {
    ...
    if (indefinite) {
        self.indefiniteTimer = [NSTimer scheduledTimerWithTimeInterval:0.001 target:self selector:@selector(updateProgress) userInfo:nil repeats:YES];
    } else {
